...
c-----------------------------------------------------      
     ...
     .     -40.*KFP**7)/DSQRT((KFP*KFP)+(MP*MP))
      ...
      SBRP = 3./2.*(DMUP*RHOP+MUP*DRHOP-(DEP+DPP))*BETA
     ...
     .     -40.*KFN**7)/DSQRT((KFN*KFN)+(MN*MN))
      ...
      SBRN = 3./2.*(DMUN*RHON+MUN*DRHON-(DEN+DPN))*BETA    
     ...
     .     -40.*KFE**7)/DSQRT((KFE*KFE)+(ME*ME))
      ...
      SBRE = 3./2.*(DMUE*RHOE+MUE*DRHOE-(DEE+DPE))*BETA
     ...
     .     -40.*KFM**7)/DSQRT((KFM*KFM)+(MO*MO))
      ...
      SBRM = 3./2.*(DMUM*RHOM+MUM*DRHOM-(DEM+DPM))*BETA
...
C      write(*,*)P,SBR!P,SBRN,SBRE,SBRM,"P,SBR"
      ...
      PT = (P/3.0D0 - U )/(HC*HC*HC)
      PRAD = (PRADS/3.0D0 - U )/(HC*HC*HC)
      PTAN = (PTANS/3.0D0 - U )/(HC*HC*HC)
      PAVG = (PAVGS/3.0D0 - U )/(HC*HC*HC)      
