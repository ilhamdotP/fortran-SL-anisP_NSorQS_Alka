...
c      RHO = 2D0*KFO*KFO*KFO/(3D0*PI*PI)
      ...
      write(filename,frmt)"EOS_FSU2HZ2_GUP", BETA, 
     & "_cek.dat"
      write(*,*)filename
...
c-----------------------------------------------------      
     ...
     .     -40.*KFP**7)/DSQRT((KFP*KFP)+(MP*MP))
      ...
      !SBRP = 3./2.*(DMUP*RHOP+MUP*DRHOP-(DEP+DPP/3.d0))*BETA
     ...
     .     -40.*KFN**7)/DSQRT((KFN*KFN)+(MN*MN))
      ...
      !SBRN = 3./2.*(DMUN*RHON+MUN*DRHON-(DEN+DPN/3.d0))*BETA    
     ...
     .     -40.*KFE**7)/DSQRT((KFE*KFE)+(ME*ME))
      ...
      !SBRE = 3./2.*(DMUE*RHOE+MUE*DRHOE-(DEE+DPE/3.d0))*BETA
     ...
     .     -40.*KFM**7)/DSQRT((KFM*KFM)+(MO*MO))
      ...
      !SBRM = 3./2.*(DMUM*RHOM+MUM*DRHOM-(DEM+DPM)/3.d0)*BETA
      
      MTOT = MUP+MUN+MUE+MUM
      DRTOT = DRHON+DRHON+DRHOE+DRHOM
      RTOT = RHON+RHON+RHOE+RHOM
      DMTOT = DMUP+DMUN+DMUE+DMUM
      DETOT = DEN+DEP+DEE+DEM
      DETOT = DETOT +GV*V0*(DRHOP+DRHON)
      DETOT = DETOT +0.5D0*GR*B0*(DRHOP-DRHON)
      DPTOT = DPN+DPP+DPE+DPM
      DPTOT = DPTOT/3.d0
      ...
      ! definisi sigma
      SBR = (MTOT*DRTOT+DMTOT*RTOT-DETOT-DPTOT)
      SBR = -(1.5d0)*BETA*SBR
      ! definisi P yang sebelumnya 
      ! adalah PRADS bukan PTANS
      PRADS = P - U
      PTANS = PRADS - SBR
