...
C------------------------------------------------------------------------------
 ...
 100  FORMAT (I3)
       WRITE(CI,100)NINT(BETA*1D8)
       
      FILENAME="basic_BSR12_SNM_GUP"//TRIM(ADJUSTL(CI))//".dat"
c      FILENAME="basic_BSR12_PNM_GUP"//TRIM(ADJUSTL(CI))//".dat"
 
 	WRITE(*,*)"File output:"
 	WRITE(*,*)FILENAME
       OPEN(unit=1,status='unknown',file=TRIM(FILENAME))
...
C     ****************************************************
       ...
       WRITE(*,*)(RB/(HC*HC*HC)),EB,KNCM,SYME,LSY,KJN0,KSY
...
C-----------------------------------------------------------------------------
     ...
     &     ,B3,C1,D2,D3,F2,G3,G4,G5,GD,MD)
      ...
      DRHS = DSQRT((KF*KF)+(MBE*MBE))
      DRHS = (3.D0*KF*MBE**5+KF**3*MBE**3-10.D0*KF**5*MBE)/DRHS
      DRHS = DRHS-3.D0*MBE**5*V
